...
public class TwoPieceBump extends SequentialCommandGroup {
  /** Creates a new OnePieceMobility. */

  public TwoPieceBump(SwerveSubsystem swerve,Arm arm,Elbow elbow,Intake intake) {
    ...
    new InstantCommand(()-> {timer.start();}), swerve.driveCommandBase(2.0, 0, 0, true).until(() -> {return timer.get()>1.8;})
    ...
    new AngleCorrection(swerve),
    swerve.driveCommandBase(1.0, 0, 0, true).until(() -> {return timer.get()>1.35;})
    .alongWith(new RunIntake(intake, Constants.Intake.coneIntakeSpeed).withTimeout(2.75)),
        new InstantCommand(()-> {timer.start();}), swerve.driveCommandBase(-3.0, 0, 0, true).until(() -> {return timer.get()>2.0;})
